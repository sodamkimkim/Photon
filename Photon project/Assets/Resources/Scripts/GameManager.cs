...
public class GameManager : MonoBehaviourPunCallbacks
{
    ...
    private void Start()
    {
        if (playerPrefab != null)
        {
            ...
            //photonView.RPC("SayThisIsMyColor", RpcTarget.All);
        }
    }
    [PunRPC]
    public void SayThisIsMyColor()
    {
    ...
    public void ApplyPlayerList()
    {
        ...
        /*
         - 현재 생성되어 있는 포톤뷰 전체와 접속중인 플레이어들의 액터 넘버를 비교해서
        플레이어 게임 오브젝트 리스트에 추가
         */
        for (int i = 0; i < playerCnt; ++i)
        {
            // 키는 0이 아닌 1부터 시작
            // 포톤에서 뭘 만들면 딕셔너리 구조로 키값이 할당되는데,
            // 그 키 값은 1부터 시작되므로 여기서도 key를 1로 초기화 했다.
            int key = i + 1;
            for (int j = 0; j < photonViews.Length; ++j)
            {
                ...
                // 접속중인 플레이어의 액터넘버 
                // 서버가 가지고 있음. OnPlayerEntertedRoom 에서 전달받은 otherPlayer에 정보 담김
